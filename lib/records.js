...
function makeRecord(data, completion = 'finished', now = new Date()) {
  ...
  return {
    ...
    sliderBreaks: Number(hits.sliderBreaks || play.hits?.sliderBreaks || 0), mods: result.mods?.name || play.mods?.name || '',
...
function selectedMapSummary(record, personality = 'balanced') {
  ...
  if (!total) return `Jamais jouée : première rencontre avec cette difficulté. Pose une référence propre — et évite de te faire présenter à la barre de vie.`;
